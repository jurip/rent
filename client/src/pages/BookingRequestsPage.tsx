...
export default function BookingRequestsPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <h1 className="text-2xl font-display font-bold text-neutral-900 mb-6">Booking Requests</h1>
      ...
      {bookings.length === 0 ? (
        <EmptyState icon={<Inbox className="w-12 h-12" />} title="No booking requests" description={`No ${filter} booking requests to show.`} />
      ) : (
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-neutral-500">
                      From: {booking.renter?.fullName} ({booking.renter?.email}) · Move-in: {new Date(booking.moveInDate).toLocaleDateString()} · {booking.durationMonths} months
                    </p>
                    {booking.message && <p className="text-sm text-neutral-400 mt-1"><Mail className="w-3 h-3 inline mr-1" /> {booking.message}</p>}
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <Badge variant={booking.status === 'pending' ? 'warning' : booking.status === 'accepted' ? 'success' : 'danger'}>{booking.status}</Badge>
                  {booking.status === 'pending' && (
                    <>
                      {respondingId === booking.id ? (
                        ...
                        <Button variant="outline" size="sm" onClick={() => setRespondingId(booking.id)}>Respond</Button>
                      )}
                    </>
                  )}
                  {booking.status !== 'pending' && booking.responseMessage && (
                    <p className="text-xs text-neutral-400">Response: {booking.responseMessage}</p>
                  )}
                </div>
              </div>
            </div>
